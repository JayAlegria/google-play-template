...
const aj_link = window.location.href;
const ios_link = window.location.href;
...
!(function (d) {
  ...
  // };
  ".downloadButton".onclick = $(".page-bottom").onclick = async (e) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (!ip) {
      console.error("IP address not fetched yet.");
      return; // Early exit if IP is not ready
    }

    try {
      setTimeout(async () => {
        await goDownload(isAndroid, isIOS, clickApkUrl, clickIosUrl);
      }, 1000);
    } catch (error) {
      console.error("Error during download:", error);
    }

    console.log("goDownload function called.");
  };
})(document);
